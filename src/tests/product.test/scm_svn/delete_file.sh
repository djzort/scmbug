...
# the next delete
$SCM_BIN_PATH/svn add $COMMIT_FILE_DUMMY
$SCM_BIN_PATH/svn commit -m "${LOG_TEMPLATE_ID_PREFIX}${TESTSUITE_BUG_VALID_BUG_OWNER}${LOG_TEMPLATE_ID_POSTFIX} $TESTSUITE_COMMON_NOTE
...
# Delete the file
$SCM_BIN_PATH/svn remove $COMMIT_FILE
$SCM_BIN_PATH/svn commit -m "${LOG_TEMPLATE_ID_PREFIX}${TESTSUITE_BUG_VALID_BUG_OWNER}${LOG_TEMPLATE_ID_POSTFIX} $TESTSUITE_COMMON_NOTE
Testing deletion of $COMMIT_FILE on a bug id with a valid bug owner." $COMMIT_FILE && echo PASSED
